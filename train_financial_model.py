...
class FinancialModelTrainer:
    ...
    def download_investopedia_dataset(self):
        ...
        try:
            # Use subprocess to run the download via datasets library
            cmd = [
                sys.executable, "-c",
                f"""
...
for split in dataset.keys():
    ...
    # Save as JSONL
    output_file = "{investopedia_dir}/investopedia_{split}.jsonl"
...
print("Investopedia dataset conversion complete!")
"""
            ]

            result = subprocess.run(cmd, capture_output=True, text=True, cwd=self.project_root)
            if result.returncode != 0:
                print(f"Error downloading Investopedia dataset: {result.stderr}")
                return False
